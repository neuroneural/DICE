...
def train_encoder(args):
    ...
    Name = args.exp + '_FBIRN_' + args.pre_training + 'DECENNT_ICA_Maxpool'
    dir = 'run-' + d1 + d2 + Name
    dir = dir + '-' + str(ID)
    wdb = 'wandb_new'
    ...
    tr_sub_HC = [15, 25, 50, 75, 135, 125] #134, 124 74
    ...
    # df = pd.read_csv(filename, header=None)
    # c_indices = df.values
    # c_indices = torch.from_numpy(c_indices).long()
    ...
    #         indices[index] = l
    #         index = index + 1
    # indices = torch.from_numpy(c_indices).long()
...
#    index_array = torch.randperm(311)
    ...
    #
    # test_indices_HC = [0, 15, 30, 45, 60, 75, 90, 105, 120, 135]
    # test_indices_SZ = [0, 16, 32, 48, 64, 80, 96, 112, 128, 144]
    #
    # test_indices_HC = [0, 38, 76, 112]
    # test_indices_SZ = [0, 40, 80, 120]

    # test_indices_HC = [0, 57, 114, 171]
    # test_indices_SZ = [0, 20, 40, 60]
...
#   LR_bottom_adjacency_matrices_learned[result_counter, :, :, :]
    ...
    # with open('../fMRI/FBIRN/AdjacencyMatrices/LR/adjacencymatrix'+str(JobID)+'.npz', 'wb') as filesim:
