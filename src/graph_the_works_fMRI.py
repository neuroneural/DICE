...
from src.utils import EarlyStopping, EarlyStoppingACC
...
class the_works_trainer(Trainer):
    def __init__(self, model, config, device, device_encoder, tr_labels, val_labels, test_labels, test_labels2="", trial="",
        ...
        self.early_stopper = EarlyStoppingACC("self.model_backup",  patience=self.patience, verbose=False,
    ...
    def do_one_epoch(self, epoch, episodes, mode):
        ...
        epoch_loss, accuracy, steps, epoch_acc, epoch_roc, epoch_roc2, epoch_prec, epoch_recall = 0., 0., 0, 0., 0., 0., 0., 0.
        ...
        for sx, ts_number in data_generator:
            ...
            logits = self.model(sx, targets, mode, self.device, epoch)



            # for i in range(FC.shape[0]):
            #     temp = torch.tanh(FC[i,:,:])
            #     t = torch.trace(temp) / 100
            #     nt = torch.sum(temp)
            #
            #     nt = nt-t
            #     diag+=t
            ...
            # loss2 = 1.0-(diag / FC.shape[0])
            # loss = (loss + (0.75*loss2))
        ...
        if epoch % 1 == 0:
          self.log_results(epoch, epoch_loss_mse / steps, epoch_loss_mi / steps, epoch_loss / steps, epoch_accuracy / steps,
        ...
        if mode == "eval" and epoch > -1:
            self.early_stopper(epoch_loss / steps, epoch_accuracy / steps, self.model, 0, epoch=epoch)
        if mode == 'test':
    ...
    def train(self, tr_eps, val_eps, tst_eps):
        ...
        # return
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=4,factor=0.5,verbose=True )
        ...
        return self.test_accuracy, self.test_auc, self.test_loss, e, self.test_precision, self.test_recall#, self.edge_weights, self.edge_weights_sum,self.attention_time#,self.test_targets#,self.test_targets2#, self.attention_weights
    ...
    def log_results(self, epoch_idx, epoch_loss_mse, epoch_loss_mi, epoch_loss, epoch_test_accuracy, epoch_accuracy2, epoch_roc,
